...
public:
	...
	//! Draws texture
	void Draw(float left = 0.0f, float top = 0.0f, float rotation = 0.0f, float scale = 1.0f, const Color& color = Color::White);
...
#define DEFINE_APP(className) \
	...
	public: \
		Initializer() \
		{ \
			extern void SetCreateAppCallbacks(App::Callbacks* (*func)()); \
			SetCreateAppCallbacks(CreateAppCallbacks); \
		} \
...
{
public:
	//! Gets random integer value in range
	static int		GetInt(int minValue = 0, int maxValue = 0x7fff);
